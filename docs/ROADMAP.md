...
| 2026-09-23 | **إصلاح CSS مفقود على الإنتاج**: `assets/dist/` (Vite build) gitignored ومش مع السحب اليدوي من hPanel Git — الصفحات كانت بترندر من غير CSS (المشكلة لقتها لأن `ui.php` بيرجع فاضي لما الـ manifest غايب). الحل: بناء محلي + rsync لـ `assets/dist` لمسار `pos.kamaltur.com` فقط — تم التحقق: `style-D-xvqDSC.css` و `app-5GlGFTuw.js` و `manifest.json` كلهم 200 + login.php بقى بيحمل الـ bundle. الـ CI بيتعامل مع ده أوتوماتيك (بيعمل `npm run build` قبل rsync) | 3 |
| 2026-09-23 | **فحص شامل على الإنتاج بحساب حقيقي (`montasar`)**: 14 صفحة كلها 200 + الشيل الجديد (`<aside>`) + صفر أخطاء PHP + **بيانات حقيقية ظاهرة**: فاتورة `260084` في القائمة، عميل `FEHMI BEJI` + عداد 207 في صفحة العملاء، وكالة `KAMAL TUR` في اللوحة. `master/dashboard.php` 302 (متوقع — مش superadmin) | 3 |
| 2026-09-23 | **تطبيق الـ Migration baseline 001 على الإنتاج** (بموافقة المستخدم): backup فوري (`pos_kamaltur_before_migrate_*.sql.gz`) → `php bin/migrate.php` → Applied 1 / Pending 0 → **البيانات بعدها مطابقة بالظبط** (207/100/135/12/2/2 + health ok + print بترسم 260084) — creates-if-not-exists فقط بلا INSERTs | 3 |
